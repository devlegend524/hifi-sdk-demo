const getRewardValue = (playedPerDayLevel, playedTimeLevel, achievementLevel, challengesLevel, userLevel, amountStakedLevel, rewardValues) => {
  if (amountStakedLevel <= 1) return Number(0).toFixed(3);
  const rewardAmount = Number(Number(rewardValues[playedTimeLevel - 1] + rewardValues[playedPerDayLevel - 1] + rewardValues[achievementLevel - 1] + rewardValues[challengesLevel - 1] + rewardValues[userLevel - 1] + rewardValues[amountStakedLevel - 1])).toFixed(3);
  return rewardAmount;
};
...
const calculateRewardLevel = (userValue, qualifyingCriteria) => {
  let parsedUserValue = Number.parseInt(userValue, 10) ? Number.parseInt(userValue, 10) : 1;
...
const getAmountStakedLevel = userStakedValues => {
  for (let index = userStakedValues.length - 1; index >= 0; index -= 1) {
    if (userStakedValues[index] && Number.parseInt(userStakedValues[index], 10) > 0) {
...
const getMultipliedRewards = (rewards, earnMultiplier, boostMultiplier) => {
  ...
  for (let index = 0; index < rewards.length; index += 1) {
    if (index < 5) {
      multipliedRewards[index] = +Number(rewards[index] * earnMultiplier).toFixed(2);
    } else {
      multipliedRewards[index] = +Number(rewards[index] * boostMultiplier).toFixed(2);
    }
  }
...
const getUserLevels = (userData, activity, stakedTokens, qualifyingCriteria) => {
  let userRewardLevels = {
    playedTimeLevel: getPlayedTimeLevel(activity.playedTime, qualifyingCriteria.playedTime),
    playedPerDayLevel: calculateRewardLevel(activity.totalPlays, qualifyingCriteria.playsPerDay),
    achievementLevel: calculateRewardLevel(userData.achievements, qualifyingCriteria.achievements),
    challengesLevel: calculateRewardLevel(userData.loginCount, qualifyingCriteria.challenges),
...
const getRewardValueFromUserLevels = (userLevels, rewardValues) => {
  const calculatedRewards = getRewardValue(userLevels.playedPerDayLevel, userLevels.playedTimeLevel, userLevels.achievementLevel, userLevels.challengesLevel, userLevels.userLevel, userLevels.amountStakedLevel, rewardValues);
  return Number(calculatedRewards).toFixed(3);
};
exports.getRewardValueFromUserLevels = getRewardValueFromUserLevels;
const getPlayRewardsForUser = (userData, activity, stakedTokens, rewards, qualifyingCriteria) => {
  return getRewardValueFromUserLevels(getUserLevels(userData, activity, stakedTokens, qualifyingCriteria), rewards);
};
exports.getPlayRewardsForUser = getPlayRewardsForUser;
const getTotalRewardsForUser = (userData, activity, stakedTokens, rewards, qualifyingCriteria, passiveRewards) => {
  const rewardsFromPlaying = getRewardValueFromUserLevels(getUserLevels(userData, activity, stakedTokens, qualifyingCriteria), rewards);
