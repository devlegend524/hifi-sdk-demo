...
const getWalletAuthHeaderObject = existingHeaders => {
  ...
  headers.append("Access-Control-Allow-Headers", "Origin, X-Origin, Content-Type, DummyAuthHeader");
...
const getSDKApiHeaderObject = async (existingHeaders, apiKey, playerId) => {
  ...
  const base64String = await window.btoa(unescape(encodeURIComponent(`${today}-${apiKey}-${playerId}-${sha256Today}`)));
...
export const apiFetchDataWithSig = async (url, method, existingOptions, existingheaders) => {
...
export const fetchSDKApi = async (url, method, existingOptions, existingheaders, apiKey, playerId) => {
  const fetchURL = `${API_URL}/${url}`;
  const headers = await getSDKApiHeaderObject(existingheaders, apiKey, playerId);
  const options = existingOptions ?? {};
  options.method = `${method}`;
  options.headers = headers;
  const res = await fetch(fetchURL, options);
