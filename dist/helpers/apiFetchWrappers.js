...
export const walletAuthFetchWithSigPrompt = async (url, method, sigPrompt, account, existingOptions, allowNoAuthOption, existingheaders) => {
  ...
  if ((!base64String || base64String === null) && !allowNoAuthOption) {
    if (sigPrompt) {
      const promptResponse = await sigPrompt(async () => {
...
export const withoutWalletAuthFetchWrapper = async (url, method, existingHeaders) => {
  const headers = existingHeaders ?? new Headers();
  headers.append('Access-Control-Allow-Origin', '*');
  headers.append('Access-Control-Allow-Methods', `${method}, OPTIONS`);
  headers.append('Access-Control-Allow-Headers', 'Origin, X-Origin, Content-Type, Authorization');
  try {
